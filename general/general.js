...
function applyUserData(userData) {
    ...
    currentProfilePic = isSafeImageSrc(userData.profilePic)
        ? userData.profilePic
        : defaultAvatar;

    if (userInfoElement) {
        userInfoElement.innerHTML =
            `<p>Registration ID: #${escapeHtml(userData.sequentialId)}</p>`;
    }

    if (profileButton && userData.sequentialId) {
        profileButton.onclick = () => {
            window.location.href =
                `/aurora/users/${encodeURIComponent(userData.sequentialId)}/profile/`;
        };
    }
}
...
function initOnlineUsersList() {
    ...
    onSnapshot(statusQuery, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            const name =
                escapeHtml(data.displayName || 'Anonymous');
            ...
            userDiv.innerHTML = `
                <div class="active-user-avatar-wrapper">
                    ...
                    >
                    <span
                        class="active-user-dot ${statusClass}"
                        title="${statusLabel}"
                    ></span>
                </div>
                <span
        ...
        if (!anyOnline) {
            onlineUsersList.innerHTML =
                `<p class="empty-state">No one else is online right now.</p>`;
        }
    });
}
...
function initChat() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        const shouldStickToBottom =
            isScrolledNearBottom(messageBox);
        ...
        docsToRender.reverse().forEach((msg) => {
            ...
            const senderDisplay =
                escapeHtml(msg.displayName || 'Anonymous');

            const senderPic =
                isSafeImageSrc(msg.profilePic)
                    ? msg.profilePic
                    : defaultAvatar;
            ...
            if (
                msg.videoUrl &&
                isSafeVideoSrc(msg.videoUrl)
            ) {
                contentHtml += `
                    <div style="margin-top: 6px;">
                        <video
                            src="${escapeHtml(msg.videoUrl)}"
...
async function sendChatMessage(text, attachment = null) {
    ...
    const command =
        text
            ? MODERATOR_COMMANDS[text.trim().toLowerCase()]
            : null;
...
async function uploadMediaToStorage(file) {
    ...
    const storagePath =
        `chat-media/${user.uid}/${uniqueName}`;
...
async function handleMediaFile(file) {
    ...
    try {
        ...
        const downloadUrl =
            await uploadMediaToStorage(file);
